<form id="client-edit-form" action="{{ route('clients.update', $client->id) }}" method="POST" class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        {{-- Colonne gauche --}}
        <div class="space-y-4">
            @foreach (['code_cli'=>'Code client','code_comptable'=>'Code comptable','societe'=>'Société','nom'=>'Nom','prenom'=>'Prénom','email'=>'Adresse e-mail','telephone'=>'Téléphone'] as $field => $label)
                <div>
            ...
            <div>
                ...
                <select name="type" id="type" class="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 shadow-sm">
            ...
            <div>
                ...
                <select name="reglement" id="reglement" class="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 shadow-sm">
            ...
            @foreach(['adresse1'=>'Adresse','adresse2'=>'Adresse 2','complement_adresse'=>"Complément d'adresse"] as $field=>$label)
                <div>
                    ...
                    <textarea name="{{ $field }}" id="{{ $field }}" rows="2"
                              class="w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 shadow-sm">{{ old($field,$client->$field??'') }}</textarea>
                    <x-input-error :messages="$errors->get($field)" />
                </div>
            @endforeach
