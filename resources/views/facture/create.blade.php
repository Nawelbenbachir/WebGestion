<form id="facture-create-form" action="{{ route('documents.store') }}" method="POST" class="text-sm">
    @csrf

    
    <input type="hidden" name="societe_id" value="{{ $parametre->derniere_societe ?? '' }}">
    <input type="hidden" name="type_document" id="type_document" value="{{ $type ?? 'facture' }}">
